...
async def _test_cold_start_leak(
    ...
    for i in range(8):
        ...
        try:
            ...
            leak_signals.extend(
                f"header:{indicator}" for indicator in _COLD_START_INDICATORS
            ...
            leak_signals.extend(
                f"body:{sig}" for sig in _COLD_START_BODY_SIGNATURES
    ...
    for payload, desc in payloads:
        ...
        try:
            ...
            timeout_signals.extend(
                f"timeout_msg:{pattern}" for pattern in _TIMEOUT_PATTERNS
