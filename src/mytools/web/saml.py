...
async def _test_assertion_replay_category(
    ...
    results.append({
        ...
        "vulnerable": False,
        "details": f"response ID={response_id[:20]}... — replay requer envio ao servidor",
        "error": "",
    })
    ...
    results.append({
        ...
        "category": "assertion_replay",
        "vulnerable": False,
        "details": "modificacao de IssueInstant — requer envio ao servidor",
        "error": "",
    })
    ...
    if not_on_after:
        results.append({
            ...
            "category": "assertion_replay",
            "vulnerable": False,
            "details": f"NotOnAfter={not_on_after} — testar se servidor valida expiracao",
            "error": "",
        })
    else:
        results.append({
            ...
            "category": "assertion_replay",
            "vulnerable": True,
            "details": "NotOnAfter ausente — assertion sem expiracao",
            "error": "",
        })
    ...
    results.append({
        ...
        "details": f"InResponseTo={in_response_to or 'ausente'} — " + ("possivel replay cross-session" if not in_response_to else "presente"),
        "error": "",
    })
...
async def _test_xml_signature_wrapping_category(
    ...
    results.append({
        ...
        "vulnerable": False,
        "details": "exclusive canonicalization attack — requer envio ao servidor",
        "error": "",
    })
    ...
    results.append({
        ...
        "details": "signature element " + ("detectado — possivel clone" if has_signature else "ausente"),
        "error": "",
    })
    ...
    results.append({
        ...
        "category": "xml_signature_wrapping",
        "vulnerable": False,
        "details": "comment injection no XML — requer envio ao servidor",
        "error": "",
    })
    ...
    results.append({
        ...
        "category": "xml_signature_wrapping",
        "vulnerable": False,
        "details": "namespace stripping — requer envio ao servidor",
        "error": "",
    })
    ...
    results.append({
        ...
        "category": "xml_signature_wrapping",
        "vulnerable": False,
        "details": "attribute manipulation apos signature — requer envio ao servidor",
        "error": "",
    })
...
async def run_scan(
    ...
    if not conditions:
        issues.append("NotBefore/NotOnAfter ausentes — assertions podem nao expirar")
    if not has_signature:
        issues.append("Assinatura XML ausente — assertions podem ser falsificadas")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="mytools-saml",
        description="SAML Attack Detection — detecta assertion replay e XML Signature Wrapping.",
...
def run_once(args: argparse.Namespace) -> int:
    ...
    if not saml_response:
        print(color("Erro: forneça um SAML Response via --saml-response ou --file", Cyber.RED))
        return 1
