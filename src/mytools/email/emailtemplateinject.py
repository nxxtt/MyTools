...
_SSTI_PROBES_DEFAULT: dict[str, str] = {
...
def _load_template_data() -> tuple[dict[str, str], dict[str, str], dict[str, list[str]]]:
    ...
    data = load_payloads("email", "template_injection", default={
        "template_payloads": _TEMPLATE_PAYLOADS_DEFAULT,
        "ssti_probes": _SSTI_PROBES_DEFAULT,
        "engine_detection": {},
    })
    return (
        data.get("template_payloads", _TEMPLATE_PAYLOADS_DEFAULT),
        data.get("ssti_probes", _SSTI_PROBES_DEFAULT),
        data.get("engine_detection", {}),
    )


_TEMPLATE_PAYLOADS, _SSTI_PROBES, _ENGINE_DETECTION = _load_template_data()
...
def scan_email_template_injection(
    ...
    return TemplateInjectionResult(
        ...
        exploit="ssti_payload_via_email" if overall == "vulnerable" else "",
