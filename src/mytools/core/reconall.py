...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "target", help="Alvo: dominio (example.com) ou URL (https://example.com)"
    )
    parser.add_argument(
        "--deep", action="store_true", help="Modo profundo (crawl, path probing)"
    )
    parser.add_argument(
        "--test-vulns", action="store_true", help="Testa XSS/SQLi no attackaudit"
    )
    parser.add_argument(
        "--test-methods",
        action="store_true",
        help="Testa metodos HTTP (PUT/DELETE/PATCH)",
    )
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Mostra mensagens de debug"
    )
    ...
    parser.add_argument(
        "--auth",
        type=parse_auth,
        help="Autenticacao Basic (user:pass). Suporta @credencial do keyring.",
    )
    parser.add_argument(
        "--bearer-token",
        dest="bearer_token",
        help="Token Bearer para autenticacao. Suporta @credencial do keyring.",
    )
    parser.add_argument(
        "--cookie", help="Cookie para as requests. Suporta @credencial do keyring."
    )
    parser.add_argument(
        "--header",
        action="append",
        default=[],
        help="Header customizado (pode repetir). Ex: 'X-Token: abc'",
    )
    parser.add_argument(
        "--skip",
        action="append",
    ...
    async def _run_all_async() -> int:
        ...
        async def _run_one(
            ...
            status = (
                ...
                if result == 0
                else color(f"FALHA ({result})", Cyber.RED, Cyber.BOLD)
            )
...
def run_once(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        logger.info(
            "Modulos: %s", ", ".join(m for m in ALL_MODULES if m not in args.skip)
        )
        if args.deep:
