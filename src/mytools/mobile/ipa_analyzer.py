        ...
        with zipfile.ZipFile(file_path, "r") as ipa:
            ...
            if info_plist_path:
                try:
                    ...
                    result["bundle_short_version"] = plist.get("CFBundleShortVersionString", "")
                    ...
                    if ats:
                        result["ats_settings"] = {
                            "allows_insecure_http": ats.get("NSAllowsArbitraryLoads", False),
                        ...
                        try:
                            ...
                            if binary is not None and hasattr(binary, "name"):
                                macho_info: dict[str, Any] = {
                                    ...
                                    "type": str(binary.header.file_type) if hasattr(binary, "header") else "",  # type: ignore[union-attr]
                                }
                                ...
                                if hasattr(binary, "exported_functions"):
                                    macho_info["exported_count"] = len(binary.exported_functions)  # type: ignore[arg-type]

                                # Symbols
                                if hasattr(binary, "symbols"):
                                    macho_info["symbol_count"] = len(list(binary.symbols))  # type: ignore[arg-type]

                                # Rpaths
                                if hasattr(binary, "rpaths"):
                                    macho_info["rpaths"] = [r.path for r in binary.rpaths if r.path]  # type: ignore[union-attr, attr-defined]
