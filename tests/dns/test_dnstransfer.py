...
class TestRunXfrScan:
    @patch("mytools.dns.dnstransfer.get_nameservers", return_value=[])
    def test_empty_ns_list(self, mock_ns):
        results = run_xfr_scan("example.com")
        assert results == []

    @patch("mytools.dns.dnstransfer.resolve_ns_to_ip", side_effect=ValueError("fail"))
...
class TestDryRun:
    ...
    def test_dry_run_no_dns_query(self):
        parser = build_parser()
        args = parser.parse_args(["example.com", "--dry-run"])
        with patch("mytools.dns.dnstransfer.get_nameservers") as mock_ns:
            result = run_once(args)
            assert result == 0
    ...
    def test_dry_run_no_zone_transfer(self):
        parser = build_parser()
        args = parser.parse_args(["example.com", "--dry-run"])
        with patch("mytools.dns.dnstransfer.try_zone_transfer") as mock_xfr:
            result = run_once(args)
            assert result == 0
